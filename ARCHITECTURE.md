...
**memberUids fan-out invariant.** Trip membership is authorized by a denormalized `memberUids` array that must be kept **identical** on the `trips/{tripId}` document and on every one of its `members`, `stops`, and `legs` documents (the security rules read `resource.data.memberUids`). Whenever membership changes, the Admin SDK write path must fan the updated set out to all of those documents — otherwise a removed member's UID lingers on the subcollection documents and they retain read access. This is centralized in `src/services/member-uids.ts`: `syncTripMemberUids()` handles add/refresh, and `removeMemberAndSyncUids()` deletes the member and fans out the post-removal set in a single atomic batch so the removal has no crash window. Both chunk their writes under Firestore's 500-write batch limit; see [docs/systems/member-uids-fan-out.md](docs/systems/member-uids-fan-out.md) for the full contract, including the atomicity guarantee and its degradation past 499 writes.
...
4. Proxy (`src/proxy.ts`) verifies the cookie on all matched routes (excluding `/_next`, `/favicon.ico`, and `/api/auth/session`) via `getAdminAuth().verifySessionCookie(cookie, true)` and redirects unauthenticated requests to `/sign-in?next=<path>`. For authenticated requests, the proxy forwards the verified `uid` to route handlers via the `x-user-id` request header.
5. Route handlers read `uid` from the `x-user-id` header (set by the proxy) — they do **not** call `verifySessionCookie` a second time.
