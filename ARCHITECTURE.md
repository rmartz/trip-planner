...
4. Proxy (`src/proxy.ts`) verifies the cookie on all matched routes (excluding `/_next`, `/favicon.ico`, and `/api/auth/session`) via `getAdminAuth().verifySessionCookie(cookie, true)` and redirects unauthenticated requests to `/sign-in?next=<path>`
