...
describe("PATCH /api/trips/[tripId]/members/[memberId]", () => {
  ...
  it("returns 403 when promoteGuestToPlanner throws planner-only error", async () => {
    vi.mocked(promoteGuestToPlanner).mockRejectedValue(
      new Error("Only Planners can promote members"),
    );
  ...
  it("returns 404 when target member not found", async () => {
    vi.mocked(promoteGuestToPlanner).mockRejectedValue(
      new Error("Target member not found or is not a Guest"),
    );
...
describe("DELETE /api/trips/[tripId]/members/[memberId]", () => {
  ...
  it("returns 403 when removeGuest throws planner-only error", async () => {
    vi.mocked(removeGuest).mockRejectedValue(
      new Error("Only Planners can remove members"),
    );
